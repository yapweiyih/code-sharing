...
def fit(df, args):
    ...
    t1 = KBinsDiscretizer(encode="onehot-dense", n_bins=10)
    t2 = StandardScaler()
    t3 = OneHotEncoder(sparse=False)

    # Must use columns name instead of index
    preprocess = ColumnTransformer(
        [
            ("t1", t1, ["age", "num persons worked for employer"]),
            ("t2", t2, ["capital gains", "capital losses", "dividends from stocks"]),
            ("t3", t3, ["education", "major industry code", "class of worker"]),
        ],
